...
public class ABOpening {
    ...
    static ArrayList<node3> immidiateMoves = new ArrayList<>(); //global array to store the first ply nodes.(all the positions after 1 move)
    static int inputPly;
    ...
    public static void main(String[] args) throws IOException {
        ...
        node3 a = (ABPmaxMin(root, Integer.MIN_VALUE, Integer.MAX_VALUE, inputPly));
    ...
    static String getAnswer(ArrayList<node3> arr, int value){
    ...
    static node3 checkMax(node3 first, node3 second){
    ...
    static node3 checkMin(node3 first, node3 second){
    ...
    static node3 ABPmaxMin(node3 x, int vMin,int vMax, int ply){
        if (ply == 0) {
            ...
            node3 temp = new node3("xxxxxxxxxxxxxxxxxxxxx");
            temp.value = Integer.MIN_VALUE;
            ArrayList<String> children = generateAdd(x.position, 'W');
            positions += children.size();
            for(int i = 0; i < children.size(); i++){
                node3 child = new node3(children.get(i));
                temp = checkMax(temp, ABPminMax(child,vMin, vMax, ply -1));
                if(temp.value >= vMax){
                    return temp;
                }
                else{
                ...
                if(ply == inputPly){
                    node3 comeOOOOn = new node3(children.get(i));
    ...
    static node3 ABPminMax(node3 x, int vMin,int vMax, int ply){
        if (ply == 0) {
            x.value = openingSestimation(x.position);
            return x;
        }
            ...
            for(int i = 0; i < children.size(); i++){
                node3 child = new node3(children.get(i));
                temp = checkMin(temp, ABPmaxMin(child,vMin, vMax, ply -1));
                if(temp.value <= vMin){
                    return temp;
                }
                else{
