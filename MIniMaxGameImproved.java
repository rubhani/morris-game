...
public class MIniMaxGameImproved {
    ...
    static ArrayList<node8> immidiateMoves = new ArrayList<>();
    static int inputPly;
    ...
    static boolean oneInMill(int location, char color, String pos) {
        return switch (location) {
            case 0 -> (pos.charAt(2) != color && pos.charAt(4) != color) && (pos.charAt(6) != color && pos.charAt(18) != color);
            case 1 -> (pos.charAt(3) != color && pos.charAt(5) != color) && (pos.charAt(11) != color && pos.charAt(20) != color);
            case 2 -> (pos.charAt(0) != color && pos.charAt(4) != color) && (pos.charAt(7) != color && pos.charAt(15) != color);
    ...
    static float ImprovedMidgameEndgameEstimation(String position){
        float whites = countPieces(position, 'W');
        float blacks = countPieces(position, 'B');
        float estimation = 0;

        float numBlackMoves = GenerateMovesMidgameEndgame(position, 'B').size();
        for (int i = 0; i < position.length(); i++){
            if(oneInMill(i,'W',position)){estimation -= 1;}
            if(oneInMill(i,'B',position)){estimation += 1;}
        }

        if (blacks <= 2) {return(10000);}
        else if (whites <= 2) {return(-10000);}
        else if (numBlackMoves==0) {return(10000);}
        else {return ( 1000*(whites - blacks) - numBlackMoves + estimation);}
    }


    //checks the final global array and returns the first highest value position
    static String getAnswer(ArrayList<node8> arr, float value){
    ...
    static node8 maxMin(node8 x, int ply) {

        if ((ply == 0) || (countPieces(x.position, 'B') <= 2)) {
            x.value = ImprovedMidgameEndgameEstimation(x.position);
            return x;
        }

        else{
            node8 temp = new node8("xxxxxxxxxxxxxxxxxxxxx");
    ...
    static node8 checkMax(node8 first, node8 second){
        float max = Math.max(first.value, second.value);
        if(max == second.value){
            first.value = max;
        }
        return first;
    ...
    static node8 checkMin(node8 first, node8 second){
        float min = Math.min(first.value, second.value);
        if(min == second.value){
            first.value = min;
        }
        return first;
    ...
    static node8 minMax(node8 x, int ply) {
