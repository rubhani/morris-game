...
public class MiniMaxOpeningImproved {
    ...
    static ArrayList<node7> immidiateMoves = new ArrayList<>(); //global array to store the first ply nodes.(all the positions after 1 move)
    static int inputPly;
    ...
    //custom estimation, key squers with more options for a mill. 0.3 is random number to give slight edge
    static float improvedEstimation(String position){
        float estimation = 0;
        for (int i = 0; i < position.length(); i++) {
            if (position.charAt(i) == 'W') {
                estimation++;
                if(i == 7 || i == 10 || i == 15 || i ==17){
                    estimation += 0.3;
                }
            } else if (position.charAt(i) == 'B') {
                estimation--;
    ...
    static String getAnswer(ArrayList<node7> arr, float value){
    ...
    static node7 checkMax(node7 first, node7 second){
    ...
    static node7 checkMin(node7 first, node7 second){
    ...
    static node7 minMax(node7 x, int ply) {
